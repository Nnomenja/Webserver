...
const std::size_t MimeTypes::TABLE_SIZE = sizeof(MimeTypes::TABLE) / sizeof(MimeTypes::TABLE[0]);

std::string MimeTypes::toLower(const std::string& s)
{
    std::string out(s);
...
std::string MimeTypes::fromExtension(const std::string& ext)
{
    ...
    for (std::size_t i = 0; i < TABLE_SIZE; ++i)
        if (lower == TABLE[i].ext)
            return TABLE[i].mime;
    return ("application/octet-stream");
}

std::string MimeTypes::fromFilename(const std::string& filename)
{
